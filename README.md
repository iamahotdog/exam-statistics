...
**此原型所有數值均為示意資料**：證書數量依使用者提供的 91–114 年圖表截圖概略重建；教育程度為 100–114 年模擬百分比，未經正式統計資料核對。圖表、資料表、CSV 及圖檔皆標示示意資料。請勿引用於分析結論、正式報告或對外統計發布。
...
教育程度模擬資料各年度六類占比總和為100%。取消勾選學歷不重新正規化，因此篩選後的堆疊高度可能低於100%。數值標籤、提示、表格與CSV均採相同基準。
