...
// Education values are illustrative percentages, in tenths of a percent.
...
].map(([year,doctor,master,bachelor,associate,high])=>({year,doctor:doctor/10,master:master/10,bachelor:bachelor/10,associate:associate/10,high:high/10,junior:(1000-doctor-master-bachelor-associate-high)/10}));
const REPORTS={
 ...
 education:{title:'公務員教育程度',section:'人事統計',short:'教育程度',eyebrow:'公務人員統計',intro:'依年度與教育程度，檢視公務員學歷組成的變化。',category:'教育程度',trend:'歷年教育程度占比',detail:'歷年教育程度明細',unit:'%',data:EDUCATION_DATA,defaultChart:'percent',series:[{key:'doctor',name:'博士',color:'#b77553',text:'#fff'},{key:'master',name:'碩士',color:'#dfb956',text:'#3c341f'},{key:'bachelor',name:'大學',color:'#278985',text:'#fff'},{key:'associate',name:'專科',color:'#83c3d0',text:'#173f48'},{key:'high',name:'高中（職）',color:'#697eb1',text:'#fff'},{key:'junior',name:'國（初）中以下',color:'#bdc9d8',text:'#263a50'}]}
};
...
function isPercentage(){return topic==='education'||$('chart-type').value==='percent'}
function value(d,s){return isPercentage()?share(d,s):d[s.key]}
function chartUnit(){return isPercentage()?'%':'張'}
...
function render(){
 ...
 $('result-title').textContent=view==='chart'?r.trend:r.detail;
 ...
 $('chart-unit').textContent='單位：'+(view==='table'&&topic==='certificates'?'張、%':chartUnit());
 ...
 $('percentage-note').textContent=topic==='certificates'?'占比＝該類證書數量 ÷ 當年兩類證書總數。篩選類別不改變分母；圖內百分比四捨五入至小數1位。':'占比以當年全部教育程度為基準，篩選類別不改變分母。較小區段的占比請於游標提示或資料表查看。';
 $('interaction-hint').textContent=window.innerWidth<700?'左右滑動查看年度，點選長條查看占比':'移至圖表，查看'+(topic==='certificates'?'數量與年度占比':'各教育程度占比');
...
function renderChart(){
 ...
 displayRows.forEach((d,i)=>{svg+=`<rect class="hit-area" data-index="${i}" tabindex="0" role="img" aria-label="民國${d.year}年，${selectedSeries.map(s=>s.name+(topic==='certificates'?fmt(d[s.key])+'張，':'')+'占比'+pct(share(d,s))).join('；')}；示意資料" x="${left+band*i}" y="${top}" width="${band}" height="${ph}" fill="transparent"/>`});
...
function showTooltip(el){
 ...
 tip.innerHTML=`<strong>民國 ${d.year} 年 <small style="color:#748390;font-weight:400">／示意</small></strong>`+selectedSeries.map(s=>`<div class="tooltip-row"><i class="swatch" style="background:${s.color}"></i>${s.name}${topic==='certificates'?`<b>${fmt(d[s.key])}</b><span class="tooltip-percent">${pct(share(d,s))}</span>`:`<b>${pct(share(d,s))}</b>`}</div>`).join('')+(topic==='certificates'?`<div class="tooltip-foot">年度兩類合計：${fmt(annualTotal(d,report()))} 張</div>`:'<div class="tooltip-foot">占比基準：當年全部教育程度</div>');
...
function renderTable(){
 $('table-head').innerHTML='<tr><th scope="col">年度（民國）</th>'+selectedSeries.map(s=>`<th scope="col">${s.name}${topic==='certificates'?'<span class="cell-share">數量／年度占比</span>':''}</th>`).join('')+`<th scope="col">${topic==='certificates'?'已選數量合計':'已選占比合計'}</th></tr>`;
 $('table-body').innerHTML=displayRows.map(d=>`<tr><td>${d.year} 年</td>${selectedSeries.map(s=>`<td>${topic==='certificates'?fmt(d[s.key])+`<span class="cell-share">${pct(share(d,s))}</span>`:pct(share(d,s))}</td>`).join('')}<td>${topic==='certificates'?fmt(total(d)):pct(selectedSeries.reduce((sum,s)=>sum+share(d,s),0))}</td></tr>`).join('');
 $('table-count').textContent=`共 ${displayRows.length} 筆年度資料 · 單位：${topic==='certificates'?'張、%':'%'} · 示意資料`;
}
...
function csvRows(){return topic==='certificates'?[['年度（民國）',...selectedSeries.flatMap(s=>[s.name+'（張）',s.name+'年度占比（%）']),'已選數量合計（張）','年度兩類總數（占比分母）','資料性質'],...displayRows.map(d=>[d.year,...selectedSeries.flatMap(s=>[d[s.key],share(d,s).toFixed(1)]),total(d),annualTotal(d,report()),'示意資料（非正式統計）'])]:[['年度（民國）',...selectedSeries.map(s=>s.name+'（%）'),'已選占比合計（%）','占比基準','資料性質'],...displayRows.map(d=>[d.year,...selectedSeries.map(s=>share(d,s).toFixed(1)),selectedSeries.reduce((sum,s)=>sum+share(d,s),0).toFixed(1),'當年全部教育程度','示意資料（非正式統計）'])]}
